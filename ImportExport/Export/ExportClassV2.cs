...
namespace Tagging
{
    public partial class ExportClassV2 : FISCA.Presentation.Controls.BaseForm, SmartSchool.API.PlugIn.Export.ExportWizard
    {
        ...
        private void wizard1_FinishButtonClick(object sender, CancelEventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                foreach (ClassRecord stu in selectedClass)
                {
        ...
        void bkwNotPassComputer_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (ManualResetEvent eve in Filler.Keys)
            {
                ...
                {
                    ClassRecord cla = Class.SelectByID(Filler[eve][0].ID);
                }
                        ...
                        if (cla != null)
                        {
                            if (RowIndex <= 65535)
                            {
                                ...
                                for (; i < studentFieldList.Count; i++)
                                {
                                    ...
                                    {
                                        case "班級系統編號": report.Worksheets[0].Cells[RowIndex, i].PutValue(cla.ID); break;
                                        case "班級名稱": report.Worksheets[0].Cells[RowIndex, i].PutValue(cla.Name); break;
                                        default:
                                            break;
                                    }
                                }
        ...
        void bkwNotPassComputer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (e.Error == null)
            {
                ...
                #region Log
                List<ClassRecord> list = Class.SelectByIDs(ClaIDList);
                foreach (ClassRecord each in list)
                {
                    SB.AppendLine("班級「" + each.Name + "」。");
                }
