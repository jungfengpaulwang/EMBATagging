...
namespace Tagging
{
    public partial class ExportCourseV2 : FISCA.Presentation.Controls.BaseForm, SmartSchool.API.PlugIn.Export.ExportWizard
    {
        ...
        public ExportCourseV2(string title, Image img)
        {
            InitializeComponent();
            ...
            _ExportableFields.ItemsChanged += delegate
            {
                ...
                List<string> newFields = new List<string>(new string[] { "課程系統編號", "學年度", "學期", "課程名稱" });
        ...
        private void wizard1_FinishButtonClick(object sender, CancelEventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                #region 取得選取課程編號
                List<CourseRecord> selectedClass = Course.SelectByIDs(K12.Presentation.NLDPanels.Course.SelectedSource);
                foreach (CourseRecord stu in selectedClass)
                {
        ...
        List<string> ClaIDList = new List<string>(); //給Log使用
        ...
        void bkwNotPassComputer_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (ManualResetEvent eve in Filler.Keys)
            {
                ...
                {
                    CourseRecord cla = Course.SelectByID(Filler[eve][0].ID);
                }
                ...
                if (RowIndex <= 65535)
                {
                    ...
                    {
                        CourseRecord cou = null;
                        if (row.ID != "")
                        {
                            if (!ClaIDList.Contains(row.ID)) //給Log使用
                            {
                                ClaIDList.Add(row.ID);
                            }

                            cou = Course.SelectByID(row.ID);
                        }
                        ...
                        if (cou != null)
                        {
                            if (RowIndex <= 65535)
                            {
                                ...
                                for (; i < studentFieldList.Count; i++)
                                {
                                    ...
                                    {
                                        case "課程系統編號": report.Worksheets[0].Cells[RowIndex, i].PutValue(cou.ID); break;
                                        case "學年度": report.Worksheets[0].Cells[RowIndex, i].PutValue(cou.SchoolYear.HasValue ? cou.SchoolYear.Value.ToString() : ""); break;
                                        case "學期": report.Worksheets[0].Cells[RowIndex, i].PutValue(cou.Semester.HasValue ? cou.Semester.Value.ToString() : ""); break;
                                        case "課程名稱": report.Worksheets[0].Cells[RowIndex, i].PutValue(cou.Name); break;
                                        default:
                                            break;
                                    }
                                }
        ...
        void bkwNotPassComputer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (e.Error == null)
            {
                ...
                #region Log
                List<CourseRecord> list = Course.SelectByIDs(ClaIDList);
                foreach (CourseRecord each in list)
                {
                    SB.AppendLine("課程「" + each.Name + "」。");
                }
                ApplicationLog.Log("類別系統.匯出", _Title, "已進行「" + _Title + "」操作。\n" + SB.ToString());
                #endregion
